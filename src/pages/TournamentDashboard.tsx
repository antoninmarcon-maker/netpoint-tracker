    ...
    return (
        <div className="space-y-2">
            <div className="flex text-xs font-bold text-muted-foreground uppercase">
                <span className="flex-1 text-center">{blue?.name ?? t('scoreboard.blueTeam')}</span>
                <span className="w-8 text-center">Set</span>
                <span className="flex-1 text-center">{red?.name ?? t('scoreboard.redTeam')}</span>
            </div>
            {scoreBlue.map((_, i) => (
            ...
            <div className="flex gap-2 mt-1">
                <button onClick={() => { setScoreBlue(p => [...p, 0]); setScoreRed(p => [...p, 0]); }}
                    className="flex-1 py-2 rounded-xl border border-border text-xs font-semibold text-muted-foreground">{t('tournaments.addSet')}</button>
                <button onClick={save} className="flex-1 py-2 rounded-xl bg-primary text-primary-foreground text-xs font-bold">{t('tournaments.saveScore')}</button>
            </div>
...
export default function TournamentDashboard() {
    ...
    const handleCreateMatch = async () => {
        if (!id || !matchBlueId || !matchRedId || matchBlueId === matchRedId) { toast.error(t('tournaments.selectDifferentTeams')); return; }
        const m = await createMatch(id, matchBlueId, matchRedId, matchRound);
        if (m) { setMatches(p => [...p, m]); setShowCreateMatch(false); setMatchBlueId(''); setMatchRedId(''); toast.success(t('tournaments.matchCreated')); }
    };
    ...
    const handleSaveScore = async (match: TournamentMatch, sb: number[], sr: number[], wid?: string) => {
        ...
        if (ok) { setMatches(p => p.map(m => m.id === match.id ? { ...m, score_blue: sb, score_red: sr, winner_id: wid ?? null } : m)); setExpandedMatch(null); toast.success(t('tournaments.scoreSaved')); }
    };
    ...
    if (!tournament) return <div className="min-h-screen bg-background flex items-center justify-center"><p className="text-foreground font-bold">{t('tournaments.notFound')}</p></div>;
    ...
    return (
        <div className="min-h-screen bg-background flex flex-col">
            <header className="px-4 py-3 pt-[max(0.75rem,env(safe-area-inset-top))] border-b border-border space-y-3">
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground">{tournament.location}{tournament.date && ` • ${new Date(tournament.date).toLocaleDateString(i18n.language)}`}</p>
                    </div>
                    {isAdmin && (
                ...
                {isAdmin && (
                    <div className="flex gap-2 overflow-x-auto pb-0.5">
                        <button onClick={() => setShowQR('join')} className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-secondary text-xs font-semibold text-muted-foreground whitespace-nowrap"><QrCode size={12} /> {t('tournaments.registrationQR')}</button>
                        <button onClick={() => setShowQR('spectate')} className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-secondary text-xs font-semibold text-muted-foreground whitespace-nowrap"><QrCode size={12} /> {t('tournaments.spectatorQR')}</button>
                        <button onClick={handleRandomDispatch} disabled={teams.length < 2} className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-secondary text-xs font-semibold text-muted-foreground whitespace-nowrap disabled:opacity-40"><Shuffle size={12} /> {t('tournaments.randomDispatch')}</button>
                        <button onClick={() => setShowCreateMatch(true)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-primary text-xs font-semibold text-primary-foreground whitespace-nowrap"><Plus size={12} /> {t('tournaments.newMatch')}</button>
                    </div>
                )}
                {isAdmin && captainRequests.length > 0 && (
                    <div className="p-3 rounded-xl border border-amber-500/30 bg-amber-500/10 space-y-2">
                        <p className="text-xs font-bold text-amber-500">{t('tournaments.captainRequests')}</p>
            ...
            <nav className="flex border-b border-border">
                {([['teams', t('tournaments.tabTeams')], ['matches', t('tournaments.tabMatches')], ['results', t('tournaments.tabResults')]] as [Tab, string][]).map(([key, label]) => (
            ...
            <main className="flex-1 p-4 max-w-2xl mx-auto w-full space-y-3">
                {tab === 'teams' && (
                    teams.length === 0
                        ? <div className="text-center py-12 text-muted-foreground text-sm">{t('tournaments.noTeams')}</div>
                        : teams.map(team => {
                                ...
                                <div key={team.id} className="bg-card border border-border rounded-2xl p-4">
                                    ...
                                    {tm.map(m => (
                                        <div key={m.id} className="flex items-center gap-1.5 text-sm text-muted-foreground">
                                            ...
                                            {m.player_name ?? t('common.player')}
                                            {m.role === 'captain_request' && <span className="text-[9px] bg-amber-500/15 text-amber-500 px-1.5 rounded-full font-bold ml-1">{t('tournaments.captainRequest')}</span>}
                                        </div>
                                    ))}
                                </div>
                            );
                ...
                {tab === 'matches' && (
                    matches.length === 0
                        ? <div className="text-center py-12 text-muted-foreground text-sm">{t('tournaments.noMatches')}</div>
                        : matches.map(match => {
                            const blue = teams.find(t => t.id === match.team_blue_id);
                            const red = teams.find(t => t.id === match.team_red_id);
                                    ...
                                    {open && (
                                        <div className="border-t border-border/50 p-4 space-y-3">
                                            {canEdit
                                                ...
                                                : <p className="text-center text-sm text-muted-foreground">{t('tournaments.readOnly')}</p>
                                            }
                                            {isAdmin && (
                    ...
                    teams.map(team => {
                        ...
                        <div key={team.id} className="flex items-center gap-3 bg-card border border-border rounded-2xl p-3">
                            ...
                            <div className="flex-1">
                                ...
                                <p className="text-xs text-muted-foreground">{t('tournaments.matchesCount', { count: p })}</p>
                            </div>
            ...
            <Dialog open={showQR !== null} onOpenChange={() => setShowQR(null)}>
                <DialogContent className="max-w-sm rounded-2xl">
                    <DialogHeader><DialogTitle className="text-center font-black">QR {showQR === 'join' ? t('tournaments.registrationQR') : t('tournaments.spectatorQR')}</DialogTitle></DialogHeader>
                    <div className="flex flex-col items-center gap-4 py-2">
                        ...
                        <button onClick={() => copy(showQR === 'join' ? joinUrl : spectateUrl)} className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-primary text-primary-foreground font-semibold text-sm w-full justify-center">
                            <Copy size={14} /> {t('heatmap.copyLink')}
                        </button>
                    </div>
                </DialogContent>
            </Dialog>
            ...
            <Dialog open={showCreateMatch} onOpenChange={setShowCreateMatch}>
                <DialogContent className="max-w-sm rounded-2xl">
                    <DialogHeader><DialogTitle className="text-center font-black">{t('tournaments.newMatch')}</DialogTitle></DialogHeader>
                    <div className="space-y-3 pt-2">
                        <select value={matchBlueId} onChange={e => setMatchBlueId(e.target.value)} className="w-full rounded-xl border border-border bg-background px-3 py-2.5 text-sm">
                            <option value="">{t('tournaments.teamA')}</option>
                        ...
                        <select value={matchRedId} onChange={e => setMatchRedId(e.target.value)} className="w-full rounded-xl border border-border bg-background px-3 py-2.5 text-sm">
                            <option value="">{t('tournaments.teamB')}</option>
                            {teams.filter(t => t.id !== matchBlueId).map(t => <option key={t.id} value={t.id}>{t.name}</option>)}
                        </select>
                        <input type="number" min={1} value={matchRound} onChange={e => setMatchRound(Number(e.target.value))} placeholder={t('tournaments.round')} className="w-full rounded-xl border border-border bg-background px-3 py-2.5 text-sm" />
                        <button onClick={handleCreateMatch} className="w-full py-3 rounded-xl bg-primary text-primary-foreground font-bold text-sm">{t('tournaments.createAction')}</button>
                    </div>
                </DialogContent>
            </Dialog>
        </div>
