...
// Exact query used by FFVB embed for beach-volley
const BEACH_QUERY = `equip_type_name = "Terrain de beach-volley" AND (equip_sol = "Sable" OR equip_sol = "Stabilisé/cendrée" OR equip_sol = "Surface naturelle" OR equip_sol = "Terre battue")`
...
function mapToSpot(r: DataEsRecord) {
  ...
  return {
    ...
    name: r.inst_nom || `Terrain de beach-volley — ${r.new_name || r.inst_cp}`,
    ...
    // lat/lng stored via PostGIS location — handled separately
...
async function runImport() {
  ...
  // Cleanup: remove previous beach import before re-importing
  console.log('Cleaning up previous ffvb_beach spots...')
  const { data: oldSpots } = await supabase
    .from('spots').select('id').eq('source', 'ffvb_beach')
  if (oldSpots && oldSpots.length > 0) {
    const ids = oldSpots.map((s: any) => s.id)
    await supabase.from('spot_photos').delete().in('spot_id', ids)
    await supabase.from('spot_comments').delete().in('spot_id', ids)
    await supabase.from('spots').delete().eq('source', 'ffvb_beach')
  ...
  return {
    ...
    skipped,
    errors,
    message: `Beach-volley import done. ${imported} upserted, ${skipped} sans coords, ${errors} erreurs.`,
  }
}
